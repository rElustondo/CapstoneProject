...
function MyComponent({userDataFromDatabase}) {
    const [contractorData, setContractorData] = useState({
      ...
      function writeUserData(userId) {
        if(contractorData.name) set(ref(db, 'users/' + user.uid + "/contractorData/name"), contractorData.name
        );
        if(contractorData.phone) set(ref(db, 'users/' + user.uid + "/contractorData/phone"), contractorData.phone
        );
        if(contractorData.location) set(ref(db, 'users/' + user.uid + "/contractorData/location"), contractorData.location
        );
                            ...
                            <TableBody>
                                {contracts.map((contract, index) => (
                                    <TableRow key={index}>
                                        ...
                                        <TableCell>{contract.recurringWeekly ? 'Yes' : 'No'}</TableCell>
        ...
        return (
            <div>
                ...
                <div>
                    <h3>Specialties: </h3>
                    <FormControlLabel
                        control={<Checkbox defaultChecked={userDataFromDatabase.contractorData.specialties.snowShoveling} onChange={(e) => setContractorData({ ...contractorData, specialties: { ...contractorData.specialties, snowShoveling: e.target.checked } })} />}
                        label="Snow Showeling"
                    />
                    <FormControlLabel
                        control={<Checkbox defaultChecked={userDataFromDatabase.contractorData.specialties.landscaping} onChange={(e) => setContractorData({ ...contractorData, specialties: { ...contractorData.specialties, landscaping: e.target.checked } })} />}
                        label="Landscaping"
                    />
                    <FormControlLabel
                        control={<Checkbox defaultChecked={userDataFromDatabase.contractorData.specialties.gardening} onChange={(e) => setContractorData({ ...contractorData, specialties: { ...contractorData.specialties, gardening: e.target.checked } })} />}
                        label="Gardening"
                    />
                    <FormControlLabel
                        control={<Checkbox defaultChecked={userDataFromDatabase.contractorData.specialties.drivewaySealing} onChange={(e) => setContractorData({ ...contractorData, specialties: { ...contractorData.specialties, drivewaySealing: e.target.checked } })} />}
                        label="Driveway Sealing"
                    />
                </div>
                <Button onClick={writeUserData} variant="contained">Save</Button>
            </div>
        );
      }
      return ( <div><h2>Total Income For Current Contracts: ${totalIncome.toFixed(2)}</h2><span>Update Contractor Profile</span><Checkbox onChange={(e) =>setUpdateProfile(e.target.checked)}></Checkbox>{updateProfile&&updateProfileForm()}{contracts(contractsDummy)}</div> )
